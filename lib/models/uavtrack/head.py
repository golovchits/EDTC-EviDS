...
class Corner_Predictor(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        with torch.no_grad():
            ...
            # generate mesh-grid
            # Non-persistent buffers follow model.to(device) but are excluded from state_dict,
            # so strict=True checkpoint loading is unaffected.
            self.register_buffer('coord_x',
                self.indice.repeat((self.feat_sz, 1)).view((self.feat_sz * self.feat_sz,)).float(),
                persistent=False)
            self.register_buffer('coord_y',
