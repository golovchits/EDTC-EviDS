...
_sys.path.insert(0, _os.path.join(_os.path.dirname(__file__), "..", "..", "..", "yolov5"))
...
from lib.test.tracker.device_config import DEVICE
...
class MixFormerOnline(BaseTracker):
    ...
    def initialize_yolo(self):
        ...
        device_str = self.cfg.YOLO.DEVICE    # cuda device, i.e. 0 or 0,1,2,3 or cpu
        # Fall back to CPU when CUDA is not available (e.g. local Mac testing)
        ...
        # For EDL checkpoints: set tau_det on the DetectEDL head so that
        # uncertainty suppression (condition c) applies during inference.
        if getattr(self.cfg.YOLO, 'EDL', False):
            tau = getattr(self.cfg.YOLO, 'TAU_DET', None)
            detect_head = model.model.model[-1]  # DetectEDL
            if hasattr(detect_head, 'tau_det'):
                detect_head.tau_det = tau
    ...
    def initialize(self, image, model):
        ...
        if len(det) == 0:
            # Miss: no detection survived NMS. Still extract mean vacuity across all
            # anchors — high vacuity on a miss is the desired signal.
            yolo_vacuity = float('nan')
            detect_head = model.model.model[-1]
            if hasattr(detect_head, '_last_vacuity'):
                yolo_vacuity = float(detect_head._last_vacuity.cpu().mean())
            return {"target_bbox": [0,0,0,0], 'uav_score': 0.0, 'detection_flag': True,
                    'yolo_conf': float('nan'), 'yolo_vacuity': yolo_vacuity}
        else:
            yolo_conf = float(det[0][4].cpu())   # EDL p_uav (or sigmoid confidence for non-EDL)
            ...
            # Extract detector vacuity for the NMS-kept detection
            yolo_vacuity = float('nan')
            detect_head = model.model.model[-1]
            if hasattr(detect_head, '_last_vacuity'):
                p_uav_all = detect_head._last_p_uav.cpu()
                vacuities = detect_head._last_vacuity.cpu()
                best_idx = (p_uav_all - yolo_conf).abs().argmin().item()
                if abs(float(p_uav_all[best_idx]) - yolo_conf) < 0.001:
                    yolo_vacuity = float(vacuities[best_idx])

            return {"target_bbox": target_bbox, 'uav_score': 1.0, 'detection_flag': False,
                    'yolo_conf': yolo_conf, 'yolo_vacuity': yolo_vacuity}
    ...
    def track(self, image, info: dict = None):
        ...
        alpha = evidence + 1
        uncertainty = 2 / torch.sum(alpha, dim=1, keepdim=True)  # uncertainty
        prob = alpha / torch.sum(alpha, dim=1, keepdim=True) # the score
        _, preds = torch.max(out_dict['pred_scores'].squeeze(1), 1) # class:target or background
        ...
        if preds == 0 or (preds == 1 and uncertainty > self.params.evidential_threshold):
            ...
            out['trk_uncertainty'] = float(uncertainty.item())  # uncertainty that triggered the switch
            return out
        ...
        if self.save_all_boxes:
            ...
            return {"target_bbox": self.state, 'uav_score': float(prob[0][1].item()),
                    'detection_flag': False, 'trk_uncertainty': float(uncertainty.item())}
